...
### POST /api/v1/text-to-speech/:model_id (Complete Audio)
...
### POST /api/v1/text-to-speech/:model_id (Streaming)
...
async function generateSpeech(modelId: string, voiceId: string, text: string) {
  const response = await fetch(`https://api.faseeh.com/api/v1/text-to-speech/${modelId}`, {
    method: 'POST',
    headers: {
...
def generate_speech(model_id, voice_id, text, api_key):
    url = f"https://api.faseeh.com/api/v1/text-to-speech/{model_id}"
    headers = {
        "x-api-key": api_key,
        "Content-Type": "application/json"
    }
